...
# scaled_X_train = scaler.fit_transform(X) # consider the distribution of the dataset that will be used
# scaled_X_train = pd.DataFrame(scaled_X_train, columns=X.columns) # transform into a dataframe and add column names
#
...
ridge = Ridge(alpha=1)  # This is lambda. If it is 0, it means it is a normal linear regression, and there is no penalty
...
# as we increase the alpha, the performance is not necessarily improved.

## 6. LASSO: It is used for feature selection
from sklearn.linear_model import Lasso

# Run LASSO with penalty = 1
lasso = Lasso(alpha=1)  # using the same penalty term
...
# Result: As the alpha increases, the performance gets worse.
...
# summarize results
print('MSE: %.3f' % mse) # The squared of bias + variance
print('Bias: %.3f' % bias)
print('Variance: %.3f' % var)

# Result: Adding bias, did not improve the performance. It actually worsens the performance.